...
| Sequential stream media | 3.2 | Generated operation-centric `.stream()` returns `OperationStream`; NDJSON/JSON Lines/JSON-seq/SSE/multipart protocols are built in; custom `StreamProtocol` and `StreamAdapter` compose through `StreamCodec`; request bodies accept `StreamSource` | typed operation stream API with protocol/adapter extension points | `internal/target/typescript/runtime_parity_test.go::TestGeneratedProviderNeutralSSEAdapterBoundaries` |
| Server-Sent Events | 3.2 | Generated standard event framing yields string `data` plus optional `event`/`id`/`retry`; application adapters may transform frames; `maxStreamFrameBytes` bounds wire events; reconnect/replay stays application-owned | `OperationStream<itemSchema>` with exported `ServerSentEvent` frame type | `internal/target/typescript/runtime_parity_test.go::TestGeneratedProviderNeutralSSEAdapterBoundaries` |
