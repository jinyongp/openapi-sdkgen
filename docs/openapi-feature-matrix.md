...
| Complete inbound body limit | all | Generated with TypeScript `--with server`: `maxBodyBytes` defaults to 8 MiB and rejects oversized complete Webhook/Callback bodies with 413 | bounded complete inbound decoding independent of streaming frame limits | `internal/target/typescript/server_body_test.go::TestGeneratedWebhookRouterDecodesTextAndFormBodies` |
