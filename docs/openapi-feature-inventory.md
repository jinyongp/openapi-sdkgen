...
| media-item-schema-and-positional-encoding | Media Type `schema`/`itemSchema` complete and incremental modes; `StreamSource` request inputs; built-in SSE/NDJSON/JSON Lines/JSON-sequence/multipart protocols; custom `StreamProtocol`/`StreamAdapter`; ordered/unnamed positional multipart with `prefixEncoding`/`itemEncoding` | 3.2 | generated | `internal/target/typescript/runtime_parity_test.go::TestGeneratedStreamingRequestSourcesAndCompleteSequentialBodies` |
...
| response-streams | Operation-centric `.stream()` returning `OperationStream`; built-in NDJSON/JSON Lines/JSON-seq/SSE/multipart protocols; built-in SSE JSON-in-`data` default; client/per-request stream codecs; custom protocols and application adapters; bounded frames and raw-response escape hatch | 3.2 | generated | `internal/target/typescript/runtime_parity_test.go::TestGeneratedResponseStreamsUseDefaultSSEJSONAdapter` |
| response-stream-lifecycle | lazy single-consumer `OperationStream`, response metadata, abort, Web Streams interop, exact-once cleanup | 3.2 | generated | `internal/target/typescript/runtime_parity_test.go::TestGeneratedOperationStreamLifecycleAndInterop` |
| stream-extension-model | `StreamProtocol`, `StreamAdapter`, `StreamCodec`, built-in SSE JSON default, client defaults, per-request override, explicit raw-frame adapter override, generated Webhook/Callback inbound reuse | 3.2 | generated | `internal/target/typescript/server_test.go::TestGeneratedWebhookRouterUsesDefaultSSEJSONAdapterAndCustomRawFrames` |
...
| complete-inbound-body-limit | `maxBodyBytes` bounds one complete generated Webhook/Callback request body while preserving streaming frame limits | all | generated | `internal/target/typescript/server_test.go::TestGeneratedWebhookRouterDecodesTextAndFormBodies` |
