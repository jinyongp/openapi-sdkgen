...
| response-streams | Operation-centric `.stream()` returning `OperationStream`; built-in NDJSON/JSON Lines/JSON-seq/SSE/multipart protocols; client/per-request stream codecs; custom protocols and application adapters; bounded frames and raw-response escape hatch | 3.2 | generated | `internal/target/typescript/runtime_parity_test.go::TestGeneratedProviderNeutralSSEAdapterBoundaries` |
...
| stream-extension-model | `StreamProtocol`, `StreamAdapter`, `StreamCodec`, client defaults, per-request override, generated Webhook/Callback inbound reuse | 3.2 | generated | `internal/target/typescript/runtime_parity_test.go::TestRuntimeStreamCodecOverridesBuiltInFraming` |
